...
namespace {
  constexpr std::array downhill_plane {
...
MOPPE_TEST (zero_age_analytical_erosion_is_identity) {
  const AnalyticalErosionResult result = erode_analytically
    (plane (), { .time_years = 0.0f, .sea_level = -1.0f });
...
MOPPE_TEST (analytical_stream_power_lowers_a_fixed_drainage_tree) {
  ...
  const AnalyticalErosionResult first = erode_analytically
    (plane (), parameters);
  const AnalyticalErosionResult second = erode_analytically
    (plane (), parameters);
