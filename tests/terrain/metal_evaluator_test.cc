...
namespace {
  ...
  void check_rasters_near (const ScalarRaster& cpu,
			   const ScalarRaster& gpu, float tolerance) {
...
MOPPE_TEST (metal_stitching_matches_pointwise_cpu_operations) {
  ...
  const ScalarField field = smoothstep
    (-0.3f, 0.8f,
     multiply_add (sin (coordinate_x ()), coordinate_y (),
		   0.25f - 1.7f * coordinate_x ()));
...
MOPPE_TEST (metal_stitching_materializes_the_geological_recipe) {
  ...
  const ScalarField field = make_geological_fields
    (make_geological_recipe (123)).combined;
  ...
  for (std::size_t y = 0; y < domain.height; ++y)
    MOPPE_CHECK_NEAR
      (gpu.at (0, y), gpu.at (domain.width - 1, y), 2e-6f);
  for (std::size_t x = 0; x < domain.width; ++x)
    MOPPE_CHECK_NEAR
      (gpu.at (x, 0), gpu.at (x, domain.height - 1), 2e-6f);
}
