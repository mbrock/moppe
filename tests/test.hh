...
namespace moppe::test {
  ...
  inline void check
    (bool condition, const char* expression,
  ...
  inline void check_near
    ...
    std::ostringstream message;
    message << location.file_name () << ":" << location.line ()
	    << ": " << actual_expression << " was " << actual
	    << ", expected " << expected_expression << " = " << expected
	    << " within " << tolerance;
    throw std::runtime_error (message.str ());
  }
}
...
#define MOPPE_CHECK(expression)                                        \
  ::moppe::test::check ((expression), #expression)
