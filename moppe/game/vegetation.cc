  ...
  namespace game {
    namespace {
      ...
      inline float grass_patch (float x, float z) {
        const float broad =
          std::sin (x * 0.036f + 1.7f * std::sin (z * 0.019f));
        const float detail = std::sin (z * 0.071f - x * 0.043f);
        return std::clamp (0.52f + 0.28f * broad + 0.20f * detail, 0.0f, 1.0f);
      }
      ...
      void record_broadleaf (render::DrawList& dl, const Spot& p, bool lean) {
        Blob blobs[4];
        ...
        for (int i = 0; i < blobn; ++i) {
          ...
          if (autumn)
            ...
            dl.color (
              0.09f + 0.10f * p.tint + j, 0.30f + 0.14f * p.tint + j, 0.09f);
          blob (dl,
                blobs[i],
                (i == 0 && !lean) ? 5 : 4,
                (i == 0 && !lean) ? 4 : 3,
      ...
      void record_birch (render::DrawList& dl, const Spot& p, bool lean) {
        Blob blobs[3];
      ...
      void record_bush (render::DrawList& dl, const Spot& p) {
        ...
        for (int i = 0; i < n; ++i) {
          const float j = r.range (-0.03f, 0.03f);
          dl.color (
            0.09f + 0.11f * p.tint + j, 0.27f + 0.15f * p.tint + j, 0.07f);
      ...
      void record_disc (render::DrawList& dl,
                        const Spot& p,
                        float radius,
                        float alpha) {
      ...
      void record_leaves (render::DrawList& dl,
                          const Spot& p,
                          const Blob* blobs,
                          int nblobs,
                          int count,
      ...
      void record_tuft (render::DrawList& dl, const Spot& p) {
        ...
        for (int k = 0; k < blades; ++k) {
          ...
          record_blade (dl,
                        ...
                        dry,
                        j,
                        std::min (1.0f, h * 1.6f) * r.range (0.45f, 0.85f),
                        3);
        }
        dl.end ();
        dl.wind (0);
      ...
      void record_bush_detail (render::DrawList& dl, const Spot& p, bool lean) {
        Blob blobs[3];
        const int n = bush_blobs (p, blobs);
    ...
    void Vegetation::generate (const map::HeightMap& map,
      ...
      for (int i = 0; i < trees * 20 && placed < trees; ++i) {
        ...
        if (yn > 0.40f)
          ...
          const float pcon =
            0.10f +
            0.75f * std::min (1.0f, std::max (0.0f, (yn - 0.08f) / 0.22f));
          species = u (rng) < pcon
                      ? Species::Conifer
                      : (u (rng) < 0.18f ? Species::Birch : Species::Broadleaf);
        }

        float scale = species == Species::Conifer ? 0.9f + 1.1f * u (rng)
                                                  : 0.9f + 1.2f * u (rng);
        if (u (rng) < 0.07f)
      ...
      for (int i = 0; i < bushes * 20 && placed < bushes; ++i) {
        ...
        const uint8_t palette =
          u (rng) < 0.30f ? (uint8_t)(1 + (int)(u (rng) * 5)) : 0;
      ...
      placed = 0;
      for (int i = 0; i < tufts * 8 && placed < tufts && !meadows.empty ();
           ++i) {
        float x, z;
        if (u (rng) < 0.68f) {
      ...
      placed = 0;
      for (int i = 0;
           i < flowers * 8 && placed < flowers && meadows.size () >= 2;
           ++i) {
        const int mi =
    ...
    void Vegetation::record_structure (render::DrawList& dl,
                                       const Plant& plant) const {
      const Spot spot = { plant.position, plant.ground_normal, plant.scale,
                          plant.tint,     plant.seed,          plant.palette };
      switch (plant.species) {
      case Species::Broadleaf:
        record_broadleaf (dl, spot, m_lean);
        break;
      case Species::Conifer:
        record_conifer (dl, spot);
        break;
      case Species::Birch:
        record_birch (dl, spot, m_lean);
        break;
      case Species::Bush:
        record_bush (dl, spot);
        break;
    ...
    void Vegetation::record_shadow (render::DrawList& dl,
                                    const Plant& plant) const {
      const Spot spot = { plant.position, plant.ground_normal, plant.scale,
                          plant.tint,     plant.seed,          plant.palette };
      switch (plant.species) {
      case Species::Broadleaf:
        record_disc (dl, spot, 1.5f * plant.scale, 0.30f);
        break;
      case Species::Conifer:
        record_disc (dl, spot, 1.2f * plant.scale, 0.30f);
        break;
      case Species::Birch:
        record_disc (dl, spot, 1.0f * plant.scale, 0.24f);
        break;
      case Species::Bush:
        record_disc (dl, spot, 0.85f * plant.scale, 0.22f);
        break;
      default:
        break;
      }
    ...
    void Vegetation::record_detail (render::DrawList& dl,
                                    const Plant& plant) const {
      const Spot spot = { plant.position, plant.ground_normal, plant.scale,
                          plant.tint,     plant.seed,          plant.palette };
      switch (plant.species) {
      case Species::Tuft:
        ...
        break;
      case Species::Flower:
        record_flower (dl, spot);
        break;
      case Species::Reed:
        record_reeds (dl, spot);
        break;
      case Species::Bush:
        record_bush_detail (dl, spot, m_lean);
        break;
      case Species::Broadleaf: {
        Blob blobs[4];
      ...
      case Species::Birch: {
        ...
        record_leaves (dl, spot, blobs, count, m_lean ? 6 : 12, false, 1.25f);
        break;
      }
      default:
        break;
    ...
    void Vegetation::load (render::Renderer& r) {
      ...
      for (int sector = 0; sector < STRUCTURE_GRID * STRUCTURE_GRID; ++sector) {
        render::DrawList dl;
        ...
        m_meshes[sector] =
          dl.empty () ? render::MeshPtr () : r.create_mesh (dl);
      }
      ...
      for (int cell = 0; cell < DETAIL_GRID * DETAIL_GRID; ++cell) {
        ...
        m_detail[cell] = dl.empty () ? render::MeshPtr () : r.create_mesh (dl);
      }
    }

    void Vegetation::render_grid (render::Renderer& r,
    ...
    void Vegetation::record_near_grass (const FrameEnv& env) {
      m_near.clear ();
      render::DrawState ds;
      ds.cull = false;
      m_near.state (ds);
      ...
      for (int gz = cz - span; gz <= cz + span; ++gz)
        for (int gx = cx - span; gx <= cx + span; ++gx) {
          ...
          uint32_t h = (uint32_t)gx * 73856093u ^ (uint32_t)gz * 19349663u;
          ...
          const float dry =
            std::min (1.0f, std::max (0.0f, (y / m_height - 0.10f) / 0.22f)) *
              0.7f +
            0.2f * ((h >> 24) * (1.0f / 255.0f)) + 0.1f * patch;
          ...
          const int blades = detailed ? 5 + (int)(r.next () * 4.0f)
                                      : 2 + (int)(r.next () * 3.0f);
          m_near.grass (true);
          ...
          for (int k = 0; k < blades; ++k) {
            ...
            const float w =
              r.range (detailed ? 0.018f : 0.025f, detailed ? 0.038f : 0.048f);
            const float j = r.range (-0.02f, 0.02f);
            record_blade (m_near,
                          root,
                          bh,
                          ...
                          dry,
                          j,
                          std::min (1.0f, bh * 1.6f) * r.range (0.45f, 0.85f),
                          detailed ? 3 : 2);
          }
          m_near.end ();
          m_near.grass (false);
