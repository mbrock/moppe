...
namespace game {
  namespace {
    void spring (Vector3D& value, Vector3D& velocity,
		 const Vector3D& target, float frequency,
  ...
  ChaseCamera::update (const Vector3D& position,
    ...
    if (reset) {
      ...
      spring (m_target, m_target_velocity, want_target,
	      2.4f, 0.92f, dt);
      spring (m_position, m_position_velocity, want_position,
	      0.95f, 0.74f, dt);
    }
  ...
  ChaseCamera::limit (const map::HeightMap& map) {
    ...
    const float target_floor = 0.35f * one_meter
