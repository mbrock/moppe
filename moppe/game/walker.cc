...
namespace game {
  ...
  void
  Walker::render (render::DrawList& dl, float time) const
  {
    ...
    const float bob = moving
      ? 0.025f * std::fabs (std::sin (ph)) : 0.0f;
    const float breathe = moving
      ? 0.0f : 0.006f * std::sin (time * 2.0f);
    ...
    for (int s = -1; s <= 1; s += 2) {
      ...
      const float knee = moving
        ? 12.0f + 30.0f * std::max (0.0f, std::sin (p - 1.1f))
        : 4.0f;
    ...
    for (int s = -1; s <= 1; s += 2) {
      ...
      dl.rotate_deg (moving
                     ? -18.0f - 10.0f * std::max (0.0f, std::sin (p))
                     : -12.0f, 1, 0, 0);
      model::rider_material (dl, model::RiderMaterial::Armor);
      dl.push ();
