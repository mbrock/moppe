...
namespace moppe::game {
  namespace {
			    ...
			    const terrain::DrainageGraph& drainage)
    {
      ...
      const bool water = census.body[cell] != terrain::LakeCensus::dry
	|| flood.ocean[cell];
      const float y = water
	? flood.water_level.values ()[cell] * scale.y
	: map.get (x, z) * scale.y;
      return Vector3D (x * scale.x, y, z * scale.z);
    }

    Vector3D cell_direction (std::uint32_t from, std::uint32_t to,
			     const map::HeightMap& map,
			     const terrain::DrainageGraph& drainage)
    {
	...
	dz = minimum_image_delta (dz, height);
      ...
      Vector3D result (dx * map.scale ().x, 0.0f,
		       dz * map.scale ().z);
    ...
    void consider (Candidate& best, std::uint32_t cell,
		   std::uint32_t from, std::uint32_t to, float score)
    {
	...
	const float score = reach.downstream_area_m2
				 ...
				 const terrain::RiverNetwork& rivers)
    {
      ...
      std::vector<std::uint32_t> upstream
	(drainage.receiver.size (), terrain::RiverReach::no_id);
      for (const terrain::RiverReach& reach : rivers.reaches) {
	if (reach.cells.empty ()
	...
	if (incoming[cell] < 2)
	  continue;
	consider (best, cell, upstream[cell], drainage.receiver[cell],
	...
	const std::uint32_t from = reach.cells.back ();
	const std::uint32_t cell = drainage.receiver[from];
	consider (best, cell, from, drainage.receiver[cell],
	...
	const float score = fall.drop_m
	  * std::sqrt (std::max (1.0f, fall.contributing_area_m2));
	...
	? selected->outlet_cell : selected->spill_cell;
      return best;
    }
  }

  std::optional<WaterShot>
  parse_water_shot (std::string_view name) noexcept
  {
    if (name == "river")
  ...
  std::string_view
  water_shot_name (WaterShot shot) noexcept
  {
    switch (shot) {
    case WaterShot::River: return "river";
    case WaterShot::Confluence: return "confluence";
    case WaterShot::Mouth: return "mouth";
    case WaterShot::Waterfall: return "waterfall";
    case WaterShot::Lake: return "lake";
    }
    return "water";
  }

  std::optional<WaterInspection>
  choose_water_inspection (WaterShot shot, const map::HeightMap& map,
			   const terrain::FloodField& flood,
			   const terrain::LakeCensus& census,
			   const terrain::DrainageGraph& drainage,
			   const terrain::RiverNetwork& rivers)
  {
    ...
    Vector3D flow = best.to != terrain::RiverReach::no_id
    ...
    Vector3D target = cell_position
      (best.cell, map, flood, census, drainage);
    float back = 30.0f;
    float sideways = 22.0f;
    float height = 16.0f;
    if (shot == WaterShot::Waterfall) {
      target = (target + cell_position
	(best.to, map, flood, census, drainage)) * 0.5f;
      back = -70.0f;
      sideways = 45.0f;
      height = 28.0f;
    ...
    Vector3D eye = target - flow * back + side * sideways
	+ Vector3D (0, height, 0);
    eye.y = std::max
      (eye.y, map.interpolated_height (eye.x, eye.z) + 4.0f);
    return WaterInspection {
      .kind = shot,
      .cell = best.cell,
      .score = best.score,
      .eye = eye,
