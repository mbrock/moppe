...
namespace moppe {
  namespace game {
    ...
    City::City ()
        : m_map (0), m_air_x0 (0), m_air_x1 (0), m_air_z0 (0), m_air_z1 (0) {}

    // -- generation ------------------------------------------------------

    void City::generate (map::RandomHeightMap& map, const WorldParams& world) {
      m_map = &map;
      ...
      for (int gy = 0; gy < H; ++gy)
        for (int gx = 0; gx < W; ++gx) {
          ...
          const float d =
            std::sqrt ((wx - cx) * (wx - cx) + (wz - cz) * (wz - cz));
          float h;
          if (d < CORE)
            h = H_CITY;
          else if (d < CORE + 500) {
      ...
      for (int i = 0; i < 6; ++i) {
        ...
        for (int gy = (int)((mz - R) / sz); gy <= (mz + R) / sz; ++gy)
          for (int gx = (int)((mx - R) / sx); gx <= (mx + R) / sx; ++gx) {
            if (gx < 0 || gy < 0 || gx >= W || gy >= H)
              continue;
            const float wx = gx * sx, wz = gy * sz;
            const float d =
              std::sqrt ((wx - mx) * (wx - mx) + (wz - mz) * (wz - mz));
            if (d >= R)
              continue;
            const float c = std::cos (1.5708f * d / R);
            const float h = H_CITY + MH * c * c;
            map.set (gx, gy, std::max (map.get (gx, gy), h / m_map_size.y));
          }
      }
      ...
      for (int i = 0; i < 30; ++i) {
        ...
        const float at = (along_x ? cx : cz) + (u (rng) - 0.5f) * 2600.0f;
        ...
        for (int gy = (int)((rz - LEN) / sz); gy <= (rz + LEN) / sz; ++gy)
          for (int gx = (int)((rx - LEN) / sx); gx <= (rx + LEN) / sx; ++gx) {
            if (gx < 0 || gy < 0 || gx >= W || gy >= H)
              continue;
            const float wx = gx * sx, wz = gy * sz;
      ...
      for (int bi = -27; bi <= 27; ++bi)
        for (int bj = -27; bj <= 27; ++bj) {
          ...
          const float d =
            std::sqrt ((bx - cx) * (bx - cx) + (bz - cz) * (bz - cz));
          if (d > CORE - 150)
            continue;
          ...
          const float hmax = 18 + 110 * std::exp (-(d / 800) * (d / 800));
          const float bh = std::max (12.0f, hmax * (0.35f + 0.65f * u (rng)));
          ...
          b.color =
            Vector3D (palette[p][0] * j, palette[p][1] * j, palette[p][2] * j);
      ...
      for (int i = 0; i < 316; ++i) {
        ...
        c.half = std::sqrt (std::max (
          1.0f, (CORE - 120) * (CORE - 120) - (k * PITCH) * (k * PITCH)));
        c.phase = 3000 * u (rng);
        ...
        static const float carpal[5][3] = {
          { 0.8f, 0.15f, 0.1f }, { 0.15f, 0.3f, 0.7f }, { 0.85f, 0.85f, 0.85f },
          { 0.9f, 0.75f, 0.1f }, { 0.1f, 0.5f, 0.45f },
        };
        const int p = (int)(u (rng) * 5) % 5;
        c.color = (c.kind == 1) ? Vector3D (0.92f, 0.92f, 0.95f)
                  : (c.kind == 2)
                    ? Vector3D (0.85f, 0.1f, 0.08f)
                    : Vector3D (carpal[p][0], carpal[p][1], carpal[p][2]);
        m_cars.push_back (c);
      }
      ...
      for (int i = 0; i < 260; ++i) {
        ...
        p.line =
          (p.along_x ? cz : cx) + k * PITCH + side * (STREET_W / 2 + 1.6f);
        p.half = std::sqrt (std::max (
          1.0f, (CORE - 160) * (CORE - 160) - (k * PITCH) * (k * PITCH)));
        ...
        p.skin = Vector3D (
          0.55f + 0.38f * tone, 0.40f + 0.32f * tone, 0.30f + 0.28f * tone);
    ...
    int City::update_people (const Vector3D& bike_pos,
                             const Vector3D& bike_vel,
                             float time) {
      if (!m_map)
        return 0;
    ...
    int City::take_car_near (const Vector3D& pos,
                             float radius,
                             float time,
                             Vector3D& out_pos,
                             Vector3D& out_dir,
      ...
      for (size_t i = 0; i < m_cars.size (); ++i) {
        ...
        c.active = false;
        out_pos = Vector3D (wx, m_map->interpolated_height (wx, wz) + 1.2f, wz);
        out_dir = c.along_x ? Vector3D (c.dir, 0, 0) : Vector3D (0, 0, c.dir);
        out_color = c.color;
        return c.kind;
      }
      return -1;
    ...
    int City::sector_of (float x, float z) const {
    ...
    bool City::in_airport (float x, float z, float margin) const {
      return x > m_air_x0 - margin && x < m_air_x1 + margin &&
             z > m_air_z0 - margin && z < m_air_z1 + margin;
    }

    void
    City::person_pos (const Person& p, float time, float& wx, float& wz) const {
      const float cx = m_map_size.x / 2, cz = m_map_size.z / 2;
      const float s = std::fmod (p.phase + p.speed * time, 2 * p.half) - p.half;
    ...
    void City::load (render::Renderer& r) {
      if (!m_map)
        return;
      ...
      for (int axis = 0; axis < 2; ++axis)
        for (int k = -20; k <= 20; ++k) {
          ...
          for (float s = -CORE; s <= CORE; s += 96) {
            ...
            if (std::sqrt ((wx - cxx) * (wx - cxx) + (wz - czz) * (wz - czz)) >
                CORE - 100)
              continue;
            if (in_airport (wx, wz, 10))
              continue;
            lamps.push_back (
              Vector3D (wx, m_map->interpolated_height (wx, wz), wz));
          }
        }
      ...
      for (int s = 0; s < SEC * SEC; ++s) {
        ...
        m_sectors[s] = dl.empty () ? render::MeshPtr () : r.create_mesh (dl);
      }

      {
        ...
        dl.state (city_state ());
        record_streets (dl);
        m_streets = dl.empty () ? render::MeshPtr () : r.create_mesh (dl);
      }

      {
        ...
        dl.state (city_state ());
        record_furniture (dl);
        m_furniture = dl.empty () ? render::MeshPtr () : r.create_mesh (dl);
      }
    }

    // streets draped over the terrain (so they follow ramps)
    void City::record_streets (render::DrawList& dl) const {
      const float cx = m_map_size.x / 2, cz = m_map_size.z / 2;
      ...
      for (int axis = 0; axis < 2; ++axis)
        for (int k = -27; k <= 27; ++k) {
          ...
          for (float s = -CORE; s <= CORE; s += 16) {
            ...
            if (std::sqrt ((wx - cx) * (wx - cx) + (wz - cz) * (wz - cz)) >
                CORE - 30) {
            ...
            const float y = m_map->interpolated_height (wx, wz) + 0.10f;
            if (axis) {
    ...
    void City::record_furniture (render::DrawList& dl) const {
    ...
    void City::emit_building (render::DrawList& dl,
                              const Building& b,
                              std::mt19937& rng) {
      ...
      dl.push ();
      dl.translate (
        (b.box.x0 + b.box.x1) / 2, H_CITY + h / 2, (b.box.z0 + b.box.z1) / 2);
      dl.scale (w, h, d);
      dl.cube (1.0f);
      dl.pop ();
      ...
      dl.push ();
      dl.translate ((b.box.x0 + b.box.x1) / 2,
                    b.box.top + 0.25f,
                    (b.box.z0 + b.box.z1) / 2);
    ...
    void City::emit_interior (render::DrawList& dl,
                              const Building& b,
                              std::mt19937& rng) {
      std::uniform_real_distribution<float> u (0.0f, 1.0f);
      ...
      // rug
      dl.color (
        0.45f + 0.4f * u (rng), 0.25f + 0.3f * u (rng), 0.3f + 0.3f * u (rng));
      dl.push ();
      ...
      for (int lx = -1; lx <= 1; lx += 2)
        for (int lz = -1; lz <= 1; lz += 2) {
          dl.push ();
          dl.translate (ix + lx * 0.75f, f + 0.35f, iz + 1.5f + lz * 0.4f);
      ...
      for (int shelf = 0; shelf < 2; ++shelf)
        for (int bk = 0; bk < 5; ++bk) {
          dl.color (0.3f + 0.65f * u (rng),
                    0.25f + 0.6f * u (rng),
                    0.3f + 0.6f * u (rng));
          dl.push ();
          dl.translate (
            ix - 2.7f + bk * 0.36f, f + 0.65f + shelf * 0.8f, b.box.z0 + 0.62f);
    ...
    void City::draw_windows (render::DrawList& dl,
                             const Building& b,
                             std::mt19937& rng) {
      std::uniform_real_distribution<float> u (0.0f, 1.0f);
      const float h = b.box.top - H_CITY;
      ...
      for (int f = 0; f < 4; ++f) {
        ...
        const float w = xf ? b.box.z1 - b.box.z0 : b.box.x1 - b.box.x0;
        ...
        const float a0 = (xf ? b.box.z0 + b.box.z1 : b.box.x0 + b.box.x1) / 2 -
                         (nc - 1) * 4.2f / 2;

        dl.normal (
          Vector3D (xf ? (float)sign : 0.0f, 0.0f, xf ? 0.0f : (float)sign));
        dl.begin (render::Prim::Quads);
    ...
    void City::emit_lamp (render::DrawList& dl, const Vector3D& p) {
    ...
    void City::emit_can (render::DrawList& dl, const Vector3D& p) {
    ...
    void
    City::draw_station (render::DrawList& dl, const mov::Box& s, bool fire) {
      ...
      for (int i = 0; i < doors; ++i) {
        dl.push ();
        dl.translate (
          mx + (i - (doors - 1) / 2.0f) * 7.5f, H_CITY + 2.3f, s.z1 - 0.1f);
      ...
      if (!fire) {
        ...
        for (int i = 0; i <= 20; ++i) {
          ...
          dl.vertex (
            mx + 5.5f * std::cos (a), s.top + 0.06f, mz + 5.5f * std::sin (a));
        }
        dl.end ();
    ...
    void City::render (render::Renderer& r,
                       render::DrawList& dl,
                       const FrameEnv& env) {
      if (!m_map)
        return;
    ...
    void City::draw_car (render::DrawList& dl,
                         const Car& c,
                         float time,
      ...
      const float cx = m_map_size.x / 2, cz = m_map_size.z / 2;
      const float s = std::fmod (c.phase + c.speed * time, 2 * c.half) - c.half;
      const float wx = c.along_x ? cx + c.dir * s : c.line;
      const float wz = c.along_x ? c.line : cz + c.dir * s;
    ...
    void City::draw_car_model (render::DrawList& dl,
                               int kind,
      ...
      box (dl, truck ? 2.1f : 1.7f, truck ? 1.5f : 0.85f, truck ? 5.6f : 3.6f);
      dl.pop ();
      ...
      if (kind != 0) {
        dl.lit (false);
        const bool phase_a = std::fmod (time * 3.0f + flash_phase, 1.0f) < 0.5f;
        for (int s = -1; s <= 1; s += 2) {
    ...
    void City::draw_heli_model (render::DrawList& dl,
                                const Vector3D& body,
                                float rotor_deg,
                                bool flash,
                                float time) {
    ...
    void
    City::draw_helicopter (render::DrawList& dl, int which, float time) const {
      const float cx = m_map_size.x / 2, cz = m_map_size.z / 2;
    ...
    void City::draw_parked_vehicles (render::DrawList& dl, float time) const {
    ...
    void City::draw_person_body (render::DrawList& dl,
                                 const Person& p,
                                 float swing) {
    ...
    void City::draw_person (render::DrawList& dl,
                            const Person& p,
                            float time,
                            const Vector3D& cam) const {
      {
    ...
    void City::draw_plane (render::DrawList& dl) {
    ...
    void City::draw_flying_plane (render::DrawList& dl,
                                  int which,
                                  float time) const {
      const float cx = m_map_size.x / 2, cz = m_map_size.z / 2;
