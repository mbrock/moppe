...
namespace moppe {
  namespace game {
    ...
    void TerrainLab::enter (render::Renderer& renderer,
      ...
      // don't rebuild. The pipeline reruns only once something is edited.
      refresh ();
    ...
    void TerrainLab::handle_build_click (float x, float y) {
      ...
      for (int row = 0; row < property_count; ++row) {
        ...
        if (domain != ParameterDomain::Natural &&
    ...
    void TerrainLab::draw_build (render::DrawList& dl) const {
      ...
      m_ui.label (dl, left_x, 632, "Select a stage to inspect | T returns");
