...
namespace moppe {
  namespace game {
    ...
    public:
      ...
      void generate (map::RandomHeightMap& map, const WorldParams& world);
      ...
      int take_car_near (const Vector3D& pos,
                         float radius,
                         float time,
                         Vector3D& out_pos,
                         Vector3D& out_dir,
      ...
      void
      render (render::Renderer& r, render::DrawList& dl, const FrameEnv& env);
    ...
    private:
      ...
      void person_pos (const Person& p, float time, float& wx, float& wz) const;
      ...
      static void emit_interior (render::DrawList& dl,
                                 ...
                                 std::mt19937& rng);
      static void
      draw_windows (render::DrawList& dl, const Building& b, std::mt19937& rng);
      static void emit_lamp (render::DrawList& dl, const Vector3D& p);
      static void emit_can (render::DrawList& dl, const Vector3D& p);
      static void
      draw_station (render::DrawList& dl, const mov::Box& s, bool fire);
      static void draw_car_model (render::DrawList& dl,
                                  int kind,
      ...
      static void
      draw_person_body (render::DrawList& dl, const Person& p, float swing);

      void record_streets (render::DrawList& dl) const;
      void record_furniture (render::DrawList& dl) const;

      void draw_car (render::DrawList& dl,
                     const Car& c,
                     float time,
                     const Vector3D& cam) const;
      void draw_person (render::DrawList& dl,
                        const Person& p,
                        float time,
                        const Vector3D& cam) const;
      void draw_helicopter (render::DrawList& dl, int which, float time) const;
      void
      draw_flying_plane (render::DrawList& dl, int which, float time) const;
      void draw_parked_vehicles (render::DrawList& dl, float time) const;
