...
int main (int argc, char** argv) {
  try {
    ...
    const std::uint32_t seed = argc > 2
      ...
      : 123;
    const ScalarField field = make_geological_fields
      (make_geological_recipe (seed)).combined;
    const Domain2D domain { .width = side, .height = side };

    const auto [cpu, cpu_seconds] = timed
      ([&] { return CpuEvaluator ().evaluate (field, domain); });
    const metal::MetalEvaluator evaluator (MOPPE_SHADER_ASSET_PATH);
    const auto [cold_gpu, cold_seconds] = timed
      ([&] { return evaluator.evaluate (field, domain); });
    const auto [warm_gpu, warm_seconds] = timed
      ([&] { return evaluator.evaluate (field, domain); });
