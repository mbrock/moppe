...
namespace moppe::terrain::metal {
  namespace {
    ...
    void check_domain (const Domain2D& domain) {
	...
	throw std::invalid_argument
	  ...
	  || domain.width > std::numeric_limits<std::size_t>::max ()
	     / domain.height)
	...
	return offset;
      ...
      const auto noise = [&] (std::uint32_t seed, int period_x,
			       int period_y, int octaves,
			       float lacunarity, float gain) {
	if (result.noises.size ()
	    >= std::numeric_limits<std::uint32_t>::max ())
	  throw std::length_error ("too many scalar field noise operations");
	const auto slot = static_cast<std::uint32_t> (result.noises.size ());
	result.noises.push_back ({
	  .permutation_offset = table_for (seed),
	  .period_x = period_x,
	  .period_y = period_y,
	  .octaves = octaves,
	  ...
	  ([&] (const auto& operation) -> PreparedNode {
	    ...
	    if constexpr (std::is_same_v<T, expression::Constant>)
	      ...
	      return { .kind = NodeKind::PeriodicFbm,
		       ...
		       .second = emit (operation.y),
		       .slot0 = noise
			 (operation.seed, operation.period_x, operation.period_y,
			  operation.octaves,
    ...
    std::runtime_error metal_error (const char* action, NSError* error) {
      ...
      message += ": ";
      message += error ? error.localizedDescription.UTF8String
	: "unknown Metal error";
  ...
  public:
	...
	if ([path.pathExtension isEqualToString:@"metal"]) {
	  ...
	  m_library = [m_device newLibraryWithURL:
  ...
  private:
	...
	if (ordinary_functions.insert (name).second)
	  [function_descriptors addObject:
	    library_function ([NSString stringWithUTF8String:name])];
	...
	(const char* base_name, const char* prefix, std::uint32_t slot) {
	  ...
	  descriptor.functionDescriptor = library_function
	    ([NSString stringWithUTF8String:base_name]);
	  descriptor.specializedName = [NSString stringWithFormat:@"%s_%u",
						 prefix, slot];
      ...
      const auto call = [&]
	(NSString* name, NSArray<id<MTLFunctionStitchingNode>>* arguments) {
	  MTLFunctionStitchingFunctionNode* node =
	    [[MTLFunctionStitchingFunctionNode alloc]
	      initWithName:name arguments:arguments controlDependencies:@[]];
	...
	return call
	  (add_specialized ("moppe_field_parameter",
			    "moppe_field_parameter", slot),
	   @[parameters]);
      };
      const auto argument = [&]
	...
	case NodeKind::Smoothstep: {
	  MTLFunctionStitchingFunctionNode* edge0 =
	    parameter_node (node.slot0);
	  MTLFunctionStitchingFunctionNode* edge1 =
	    parameter_node (node.slot1);
	  lowered_node = call (add_ordinary ("moppe_field_smoothstep"),
	...
	case NodeKind::PeriodicRidged: {
	  ...
	  switch (node.kind) {
	  case NodeKind::Perlin:
	    base = prefix = "moppe_field_perlin"; break;
	  case NodeKind::Fbm:
	    base = prefix = "moppe_field_fbm"; break;
	  case NodeKind::Ridged:
	    base = prefix = "moppe_field_ridged"; break;
	  case NodeKind::PeriodicFbm:
	    base = prefix = "moppe_field_periodic_fbm"; break;
	  case NodeKind::PeriodicRidged:
	    base = prefix = "moppe_field_periodic_ridged"; break;
	  default: break;
	  }
	  lowered_node = call (add_specialized (base, prefix, node.slot0),
	    @[argument (lowered, node.first),
	      argument (lowered, node.second), noises, permutations]);
	  break;
	}
	}
	lowered.push_back (lowered_node);
      ...
      MTLFunctionStitchingFunctionNode* output = call
	(add_ordinary ("moppe_field_finish"),
	...
	[[MTLFunctionStitchingGraph alloc]
	  ...
	  nodes:graph_nodes outputNode:output
	...
	[MTL4ComputePipelineDescriptor new];
      ...
      NSError* error = nil;
      id<MTLComputePipelineState> pipeline = [m_compiler
  ...
  MetalEvaluator& MetalEvaluator::operator = (MetalEvaluator&&) noexcept =
    default;

  ScalarRaster MetalEvaluator::evaluate
