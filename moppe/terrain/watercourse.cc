...
namespace moppe::terrain {
  namespace {
    ...
    float wave_amplitude (const FloodField& flood,
			  const LakeCensus& census, std::size_t cell) {
      if (flood.ocean[cell])
	return 1.0f;
	...
	return 0.0f;
      ...
      case WaterBodyClass::Sea: return 1.0f;
      case WaterBodyClass::Lake: return 0.10f;
      case WaterBodyClass::Pond: return 0.04f;
      case WaterBodyClass::Puddle: return 0.0f;
      }
      return 0.0f;
    }
  }
  ...
  WaterSheets
  paint_watercourses (const TerrainView& terrain, const FloodField& flood,
		      ...
		      const WatercoursePaint& parameters)
  {
    ...
    const ScalarRaster permanent = permanent_water_surface
      (flood, census, parameters.permanence);
	...
	const float rapid = rapid_signal
	  (drainage.slope.values ()[slope_cell]);
	const float fall = rivers.waterfall_by_cell[cell]
	  != Waterfall::no_id ? 1.0f : 0.0f;
	points.push_back ({
	  .cell = cell,
	  .level_m = 0.0f,
	...
	for (const float extent : { 0.7f, 1.0f, 1.3f }) {
	  int x = static_cast<int> (point.cell) % width
	    + static_cast<int> (std::lround
	      (side * point.direction_z * extent * probe / grid.spacing_x));
	  int y = static_cast<int> (point.cell) / width
	    + static_cast<int> (std::lround
	      (-side * point.direction_x * extent * probe / grid.spacing_y));
	  if (periodic) {
	...
	if (point.water)
	  ...
	  const float carved = std::min (bank_m (point, 1.0f),
					 bank_m (point, -1.0f)) - ground;
	  const float law = channel_depth_m
	    (drainage.contributing_area.values ()[point.cell]);
	  target = ground + parameters.channel_fill
	...
	surface[cell] = std::max (surface[cell],
				  river_level[cell] / height_scale);
    ...
    return {
      .surface = ScalarRaster (permanent.domain (), std::move (surface)),
