...
namespace moppe::terrain {
  ScalarRaster
  analyze_moisture (const FloodField& flood, const LakeCensus& census,
		    ...
		    const MoistureParameters& parameters)
  {
	...
	: steps[cell] * spacing;
      const float near_water = std::exp
	(-distance / parameters.water_reach_m);
      const float area = drainage.contributing_area.values ()[cell];
      const float damp = std::clamp
	(std::log2 (std::max (area / cell_area, 1.0f))
	 / parameters.drainage_span_log2, 0.0f, 1.0f);
      moisture[cell] = std::clamp
	((1.0f - parameters.drainage_weight) * near_water
	 + parameters.drainage_weight * damp, 0.0f, 1.0f);
    }
    return ScalarRaster
