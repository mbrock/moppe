...
namespace {
  ...
  void write_map (const std::string& path,
    ...
    const std::size_t count = static_cast<std::size_t> (map.width ())
      * map.height ();
...
int main (int argc, char** argv) {
  ...
  try {
    ...
    const HydraulicErosion hydraulic {
      .droplets = droplets,
      .batch_size = 256,
      .max_steps = 512,
      .minimum_water = 0.01f,
	...
	if (const auto* report = std::get_if<HydraulicErosionReport> (&result))
      ...
      const float cell_area = drainage.source_grid.spacing_x
	* drainage.source_grid.spacing_y;
	...
	if (body.classification == WaterBodyClass::Puddle) ++puddles;
	if (body.classification == WaterBodyClass::Pond) ++ponds;
	if (body.classification == WaterBodyClass::Lake) ++lakes;
	if (body.classification != WaterBodyClass::Sea)
	  water += body.volume_m3;
      }

      std::cout << mode.name << ',' << total_ms
