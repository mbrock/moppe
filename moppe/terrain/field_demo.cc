...
namespace {
  moppe::terrain::ScalarField wave_field () {
    ...
    return 0.5f + 0.25f * sin ((3.0f * tau) * x) +
           0.25f * sin ((2.0f * tau) * y);
  }

  moppe::terrain::ScalarField preset_field (std::string_view preset,
                                            std::uint32_t seed) {
    using namespace moppe::terrain;
    if (preset == "waves")
      return wave_field ();
    ...
    if (!layer)
      throw std::invalid_argument ("unknown preset: " + std::string (preset));
...
int main (int argc, char** argv) {
  ...
  try {
    ...
    std::cout << "wrote " << path << " (" << preset << ", seed " << seed << ", "
              << resolution << "x" << resolution << ", "
              << unique_node_count (field) << " DAG nodes, range "
              << raster.min_value () << ".." << raster.max_value () << ")\n";
