...
namespace moppe::terrain {
        ...
        [&visited] (const auto& operation) {
          ...
          if constexpr (std::is_same_v<T, expression::Add> ||
            ...
            visit_nodes (operation.right, visited);
                               ...
                               std::is_same_v<T,
                                              expression::PeriodicFbmNoise> ||
                               std::is_same_v<
                                 T,
                                 expression::PeriodicRidgedNoise>) {
  ...
  ScalarField
  smoothstep (float edge0, float edge1, const ScalarField& operand) {
    if (!(edge1 > edge0))
  ...
  namespace {
    void validate_fractal_noise (int octaves, float lacunarity, float gain) {
      if (octaves <= 0)
  ...
  namespace {
    void validate_periodic_noise (
      int period_x, int period_y, int octaves, int lacunarity, float gain) {
      if (period_x <= 0 || period_y <= 0)
        throw std::invalid_argument ("periodic noise periods must be positive");
  ...
  ScalarField periodic_fbm_noise (Seed seed,
                                  ...
                                  int lacunarity,
                                  float gain) {
    validate_periodic_noise (period_x, period_y, octaves, lacunarity, gain);
    return make_field (expression::PeriodicFbmNoise { seed,
                                                      x.node (),
                                                      y.node (),
                                                      period_x,
                                                      period_y,
  ...
  ScalarField periodic_ridged_noise (Seed seed,
                                     ...
                                     float gain) {
    validate_periodic_noise (period_x, period_y, octaves, lacunarity, gain);
    return make_field (expression::PeriodicRidgedNoise { seed,
                                                         x.node (),
                                                         y.node (),
                                                         period_x,
                                                         period_y,
  ...
  ScalarField operator+ (const ScalarField& left, const ScalarField& right) {
    return make_field (expression::Add { left.node (), right.node () });
  }

  ScalarField operator* (const ScalarField& left, const ScalarField& right) {
    return make_field (expression::Multiply { left.node (), right.node () });
  }

  ScalarField operator- (const ScalarField& left, const ScalarField& right) {
    return make_field (expression::Subtract { left.node (), right.node () });
  }
  ...
  NoiseField ridged_noise (Seed seed,
                           ...
                           float lacunarity,
                           float gain) {
    return NoiseField (ridged_noise (
  ...
  NoiseField periodic_fbm_noise (Seed seed,
                                 ...
                                 int lacunarity,
                                 float gain) {
    return NoiseField (periodic_fbm_noise (seed,
                                           x.untyped (),
                                           y.untyped (),
                                           period_x,
                                           period_y,
  ...
  NoiseField periodic_ridged_noise (Seed seed,
                                    ...
                                    int lacunarity,
                                    float gain) {
    return NoiseField (periodic_ridged_noise (seed,
                                              x.untyped (),
                                              y.untyped (),
                                              period_x,
                                              period_y,
