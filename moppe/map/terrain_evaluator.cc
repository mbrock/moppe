...
namespace moppe::map {
  TerrainEvaluator::TerrainEvaluator (
    ...
    const terrain::StreamPowerEvolutionBackend* evolution_backend)
      : m_target (target), m_evolution_backend (evolution_backend) {}
  ...
  void TerrainEvaluator::begin (const terrain::TerrainProgram& program,
    ...
    m_uplift = spatial::get<terrain::uplift_weight> (geology);
    ...
    for (std::size_t offset = 0; offset < geology.size (); ++offset) {
      ...
      elevations[offset] = SurfaceElevation (
  ...
  TerrainEvaluator::apply (const terrain::TerrainTransform& transform) {
    ...
    if (const auto* orogeny =
      ...
      if (m_uplift.size () != sample_count)
        throw std::logic_error (
          "orogeny evolution requires geology generated by begin");
      ...
      uplift.reserve (m_uplift.size ());
      for (const terrain::UpliftWeight weight : m_uplift)
        uplift.push_back (weight.numerical_value_in (one) * maximum_uplift *
                          mp_units::si::metre /
