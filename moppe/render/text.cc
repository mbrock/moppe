...
namespace render {
  namespace {
	...
	if (out_x) { out_x[i] = pen_x; out_y[i] = pen_y; }
	if (h > row_h)
	  row_h = h;
	pen_x += w + spacing;
	any = true;
  ...
  FontAtlas::FontAtlas (Renderer& renderer, const char* family,
      ...
      m_scale (scale > 0.0f ? scale : 1.0f)
  {
    ...
    for (int i = 0; i < GLYPH_COUNT; ++i)
      ok[i] = platform::rasterize_glyph
	(family, point_size, m_scale,
	 (unsigned int) (FIRST_CHAR + i), bitmaps[i]);
    ...
    while (atlas_w < max_w + 2)
      atlas_w *= 2;
    while (shelf_pack (bitmaps, ok, GLYPH_COUNT, atlas_w, 0, 0)
	     > atlas_w
	   && atlas_w < 2048)
      atlas_w *= 2;
	...
	for (int x = 0; x < bm.width; ++x) {
	  const unsigned char c =
	    bm.pixels[(size_t) y * bm.width + x];
	  unsigned char* dst =
	    &pixels[((size_t) (ys[i] + y) * atlas_w
		     + (xs[i] + x)) * 4];
  ...
  float
  FontAtlas::draw (DrawList& dl, float x, float y,
		   const std::string& text) const
  {
	...
	dl.uv (g.u0, g.v0); dl.vertex (x0, y0);
	dl.uv (g.u1, g.v0); dl.vertex (x1, y0);
	dl.uv (g.u1, g.v1); dl.vertex (x1, y1);
	dl.uv (g.u0, g.v1); dl.vertex (x0, y1);
      }
      pen += g.advance;
    }
    dl.end ();
  ...
  float
  FontAtlas::measure (const std::string& text) const
  {
    float w = 0;
    for (size_t i = 0; i < text.size (); ++i) {
      const unsigned char c = (unsigned char) text[i];
