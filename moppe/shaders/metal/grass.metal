...
static GrassBlade
grass_blade (uint world_cell_x, uint world_cell_z, uint blade,
  ...
  const float2 world_xz = (float2 (int2 (world_cell_x, world_cell_z))
			   + 0.5 + jitter * 0.92) * grass.grid.z;
  ...
  b.root = float3 (world_xz.x, terrain_height * grass.terrain.y,
		   world_xz.y);
  ...
  if (valid && b.distance > 12.0) {
    const float4 clip = frame.view_proj
      * float4 (b.root + float3 (0.0, 0.6, 0.0), 1.0);
  ...
  b.tip_color = mix (b.base_color, float3 (0.62, 0.76, 0.22),
		     0.42 + 0.30 * patch);
  return b;
}

static GrassVaryings
grass_blade_vertex (const GrassBlade b, float t, float side_sign,
  ...
  float3 world = b.root
    ...
    + float3 (b.side.x * width * side_sign, 0,
	      b.side.y * width * side_sign);
...
using GrassMesh = metal::mesh<GrassVaryings, void,
			      GRASS_PATCH_BLADES * 8,
			      GRASS_PATCH_BLADES * 6,
			      metal::topology::triangle>;

[[object]] void
grass_object (object_data GrassPayload& payload [[payload]],
  ...
  if (valid) {
    ...
    const float2 center_cell = origin
      + float2 (patch_x * GRASS_PATCH_X + 0.5 * GRASS_PATCH_X,
		patch_z * GRASS_PATCH_Z + 0.5 * GRASS_PATCH_Z);
    const float2 world_xz = center_cell * grass.grid.z;
    const float patch_radius = 0.5 * grass.grid.z
      * length (float2 (GRASS_PATCH_X, GRASS_PATCH_Z)) + 1.6;
	...
	grid -= floor (grid / terrain_limit) * terrain_limit;
      ...
      valid = height > grass.limits.x - slack
	&& height < grass.limits.y + slack;

      if (valid && distance > 12.0) {
	const float4 clip = frame.view_proj
	  * float4 (world_xz.x, height * grass.terrain.y + 0.6,
		    world_xz.y, 1.0);
...
grass_mesh (GrassMesh out,
  ...
  const int2 origin = int2 (grass.grid.xy);
  const uint world_cell_x = uint
    (origin.x + int (patch.x * GRASS_PATCH_X + cell % GRASS_PATCH_X));
  const uint world_cell_z = uint
