...
static float3 sky_atmosphere (float3 ray_dir, float3 sun_dir,
  ...
  if (sign_zenith >= 0.0) {
    const float3 zenith_color = mix (night_zenith, day_zenith,
				     daylight);
    const float3 mid_color = mix (night_mid, day_mid, daylight);
    const float3 horizon_color = mix (night_horizon, day_horizon,
				      daylight);
    ...
    base_color = t < 0.62
      ? mix (zenith_color, mid_color, t / 0.62)
...
static float sky_cloud_shape (float3 p, float coverage, float time) {
  ...
  return smoothstep (cloud_coverage, cloud_coverage + 0.13,
		     base + detail * 0.2);
}
...
static float3 sky_cloud_lighting (float cloud_density, float3 view_dir,
				  float3 sun_dir, float sun_height) {
  (void) sun_height;
  const float daylight = smoothstep (-0.08, 0.18, sun_dir.y);
  const float golden = daylight
    * (1.0 - smoothstep (0.15, 0.65, sun_dir.y));
  ...
  const float toward_sun = pow (max (dot (view_dir, sun_dir), 0.0),
				16.0);
  cloud_color += lit * toward_sun * (1.0 - core)
    * (0.5 + 0.7 * golden) * daylight;
	      ...
	      constant MoppeSkyUniforms& u [[buffer(MOPPE_BUF_FRAME)]])
{
  ...
  if (dir.y > 0.08) {
    ...
    float streak = sky_fbm (float3 (ci.x * 0.0035, ci.y * 0.008,
				    ci.z * 0.0065));
  ...
  if (dir.y > 0.05) {
    ...
    if (clouds > 0.01) {
      float3 cloud_color = sky_cloud_lighting (clouds, dir, sun,
					       sun_height);
