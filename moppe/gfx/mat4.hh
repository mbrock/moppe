...
namespace moppe {
  ...
  struct Mat4 {
    ...
    static Mat4 basis (const Vector3D& x, const Vector3D& y,
		       const Vector3D& z,
    ...
    static Mat4 look_at (const Vector3D& eye, const Vector3D& center,
			 const Vector3D& up) {
    ...
    static Mat4 perspective_reversed (radians_t fovy, float aspect,
    ...
    static Mat4 ortho (float l, float r_, float b, float t,
		       float near, float far) {
      Mat4 r;
      r.m[0] = 2 / (r_ - l);
      r.m[5] = 2 / (t - b);
  ...
  struct NormalMat {
    ...
    Vector3D apply (const Vector3D& v) const {
      return Vector3D (c0.x*v.x + c1.x*v.y + c2.x*v.z,
		       ...
		       c0.z*v.x + c1.z*v.y + c2.z*v.z).normalized ();
    }
  };
}
