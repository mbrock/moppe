...
namespace tga {
  ...
  int TGAImg::ReadHeader () // Examine the header and populate our class
                            // attributes
  {
    ...
    if (pData[1] >
        1) // 0 (RGB) and 1 (Indexed) are the only types we know about
      return IMG_ERR_UNSUPPORTED;
    ...
    // Reject indexed images if not a VGA palette (256 entries with 24 bits per
    // entry)
  ...
  int TGAImg::LoadTgaRLEData () // Load RLE compressed image data
  {
    ...
    while (Index < lImageSize) {
      ...
      } else                // Raw chunk
      {
        ...
        for (bLoop = 0; bLoop != bLength;
             ++bLoop, Index += iPixelSize, pCur += iPixelSize)
          memcpy (&pImage[Index], pCur, iPixelSize);
      }
    }
  ...
  void TGAImg::FlipImg () // Flips the image vertically (Why store images upside
                          // down?)
  {
    unsigned char bTemp;
