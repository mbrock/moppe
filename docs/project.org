...
** TODO Turn the water network into visible reaches

Body-level routing now has enough identity for its first visible consumer.
Extract coherent stream reaches above a configurable contributing-area
threshold, stop them at lake inlets, and resume them at spills.  Preserve
reach direction, slope, and discharge as rendering inputs before generating
or carving any river geometry.

Acceptance evidence:

- Reaches join at confluences and do not draw bookkeeping paths across lakes.
- Every reach retains direction, contributing area, and bed slope.
- The extracted network is deterministic, periodic, and inspectable.
- Extraction remains a reading and does not silently edit terrain.
...
- Consolidated submerged cells into one outlet tree per connected sea.
